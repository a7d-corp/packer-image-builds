proxmox_insecure_tls = true
proxmox_iso_storage_pool = "nfs-vm-images"
proxmox_iso_storage_type = "iso"
iso_template_name = "ubuntu-24.04.1-live-server-amd64.iso"
iso_checksum = "sha512:3d518612aabbdb77fd6b49cb55b824fed11e40540e4af52f5f26174257715c93740f83079ea618b4d933081f0b1bc69d32b7885b7c75bc90da5ad3fe1814cfd4"
template_description = "Ubuntu Noble Numbat (2404)"
template_name = "template-ubuntu-2404"